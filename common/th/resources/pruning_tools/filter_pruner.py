...
def prune_layers(net, prune_targets, prune_all=True, device='cpu'):
    ...
    for layer_index, channel_index in prune_targets:
        ...
        if layer_index == 3 and conv.out_channels <= 32:
            print('!!!NOTICE!!! ({},{}) not pruned. Layer 3 is already in lowest threshold 48'.format(layer_index, channel_index));
            # print("new_conv", new_conv)
        elif 0 in new_conv.weight.shape:
            continue

        elif conv.out_channels <= 8:
            print(f'Skipping pruning for layer {layer_index} as it would reduce channels below 4')
            continue

        else:
            #remove output channel from conv layer
            if (new_conv == 0) or (new_bn == 0):
                print("new_conv", new_conv, "new_bn", new_bn)
                continue;
            else:    
                all_features[layer_index] = (conv_idx, new_conv);
...
def get_new_weight(weight, dim, channel_index):
    indexes = list(set(range(weight.size(dim))) - set({channel_index}))
    # Create tensor with dtype=torch.int64
    new_data = torch.index_select(weight, dim, torch.tensor(indexes, dtype=torch.int64))
    return new_data
